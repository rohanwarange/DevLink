...
public class UserService {
    ...
    public User findById(int id) {
        User user = userRepository.findById(id).orElseThrow(() -> 
            new DataNotFoundException("Could not find user with id: " + id));
        return user;
    }
    ...
    public User getCurrentUser() {
        ...
        if (authentication != null && authentication.getPrincipal() instanceof User) {
            ...
            User customUser = userRepository.findByUsername(principal.getUsername()).orElseThrow(() ->
                new DataNotFoundException("Could not find user with username: " + principal.getUsername()));
            return customUser;
        }
        return null;
    }
    ...
    // Disconnect and set status to OFFLINE
    ...
    public List<User> findUsersChattedWith(String userId) {
        // Retrieve all chat rooms of the current user
        List<ChatRoom> chatRooms = chatRoomService.getChatRoomsBySenderId(userId);

        // Filter out chat rooms that the current user has interacted with
        List<User> users = chatRooms.stream()
                .map(chatRoom -> {
                    String recipientId = chatRoom.getRecipientId();
                    return userRepository.findByUsername(recipientId).orElseThrow(() ->
                        new DataNotFoundException("Could not find user with recipientId: " + recipientId));
                })
                .collect(Collectors.toList());

        // Return the list of users the current user has chatted with
        return users;
    }
